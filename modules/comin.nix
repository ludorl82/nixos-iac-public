...
# own nixosConfiguration (matched by hostname) and switches when master moves.
# Merge to master IS the deploy; PR review (the eval check in
...
# merge to master is deployed by comin on enrolled hosts (vm-01 so far).
