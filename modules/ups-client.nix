...
# gpu-01 is the master (modules/ups-master.nix); at low battery gpu-01's upsd
...
# must be byte-identical to the master's copy — generate once, `sudo tee`
...
let
  ...
  # Same ntfy hook as ups-master.nix: ONLINE/COMMOK resolve, rest is high.
...
in
{
  power.ups = {
    ...
    upsmon = {
      monitor.qnapups = {
        system = "qnapups@192.0.2.129"; # gpu-01, the rolling-rack master
        user = "monclient";
